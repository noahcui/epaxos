...
func (r *Replica) skipFor(instanceID int32) {
	r.instanceSpace[instanceID] = &Instance{true,
		100,
		nil,
		0,
		COMMITTED,
	...
	r.updateBlocking(instanceID)
	for i := 0; i < r.N; i++ {
		if int32(i) != r.Id && int32(i) != r.takeOver {
			r.bcastSkip(instanceID, instanceID+99, int32(i))
		}
	}
...
func (r *Replica) handletakeoverAccept(accept *menciusoptproto.Accept) {
	flush := true
	crt := r.crtInstance + r.takeOver - r.Id
	skipStart := int32(-1)
	skipEnd := int32(-1)
	if accept.Skip == FALSE && crt < accept.Instance {
		skipStart = crt
		skipEnd = accept.Instance/int32(r.N)*int32(r.N) + r.takeOver
		...
		r.instanceSpace[crt] = &Instance{true,
			int(skipEnd-crt)/r.N - 2,
			nil,
			-1,
			COMMITTED,
			nil,
		...
		r.recordInstanceMetadata(r.instanceSpace[crt])
		r.sync()
	...
	if preply.OK == TRUE {
		...
		if inst.lb.prepareOKs+1 > r.N>>1 {
			...
			if inst.nbInstSkipped < 0 {
				...
				// give it some
				fmt.Println("server", r.Id, "took over for", int(r.Id+1)%r.N)
				inst.nbInstSkipped = -inst.nbInstSkipped
				// r.skipFor(preply.Instance)
			}
			// } else {
...
func (r *Replica) handleAcceptReply(areply *menciusoptproto.AcceptReply) {
	...
	if areply.OK == TRUE {
		...
		// TODO: there is probably another active leader
		log.Println(r.Id, areply.Instance, areply.Instance%int32(r.N), inst.lb)
		inst.lb.nacks++
		...
		if (areply.Ballot&0x0F)%int32(r.N) == areply.Instance%int32(r.N) {
			...
			log.Println("the owner of the instance is trying to commit something, I should give up")
		}
		...
		for i := execedUpTo + 1; i < r.crtInstance; i++ {
			...
			if r.instanceSpace[i].status != COMMITTED {
				...
				if !r.instanceSpace[i].skipped && r.instanceSpace[i].commands != nil {
					if len(r.instanceSpace[i].commands) == 0 {
						continue
					}
...
func (r *Replica) forceCommit() {
	...
	if r.takeOver >= 0 && int(problemInstance)%r.N == int(r.takeOver) {
		r.broadAccpetFor(problemInstance)
		// r.skipFor(problemInstance)
