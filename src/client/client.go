...
func main() {
	...
	for j := 0; j < *rounds; j++ {
		...
		if *noLeader {
			for i := 0; i < N; i++ {
				go waitReplies(writers, readers, i, perReplicaCount[i], done)
			}
		} else {
			go waitReplies(writers, readers, leader, n, done)
		}
...
func waitReplies(writers []*bufio.Writer, readers []*bufio.Reader, leader int, n int, done chan bool) {
	e := false
